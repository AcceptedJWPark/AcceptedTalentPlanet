...
public class CustomerService_Manual_AnswerList_Activity extends AppCompatActivity {
    ...
    private void setArrayData(String value)
    {
        if (value.equals("MyProfile"))
        {
            ...
            arrayGroup.add("재능 드림 / 관심 재능");
            arrayList7.add("재능 드림, 관심 재능은 가입 이후 필수 입력 사항입니다. 재능 입력이 안된 상태에서 Talented 사용이 불가합니다. \"좌측 상단의 슬라이드 메뉴\" → \"My Talent\"에서 재능 등록을 꼭 해주세요.");
        ...
        else if (value.equals("MyTalent"))
        {
            ...
            ArrayList<String> arrayList2 = new ArrayList<String>();
            arrayGroup.add("재능 등록");
            arrayList2.add("재능 등록 키워드는 최대 3개 입력이 가능합니다. 세 개의 키워드는 겹치지 않게 다른 키워드로 입력을 해야 매칭 확률이 높아집니다.\n 회원님의 관심 재능(재능 드림)과 상대방의 재능 드림(관심 재능)이 매칭 되어 T.Sharing에 나열 됩니다.");
            arrayChild.put(arrayGroup.get(1),arrayList2);

            ArrayList<String> arrayList3 = new ArrayList<String>();
            arrayGroup.add("장소 등록");
            arrayList3.add("장소 등록은 최대 3개 입력이 가능합니다. 세 개의 장소는 겹치지 않게 다른 장소들을 입력해야 매칭 확률이 높아집니다.\n T.Sharing에서는 회원님이 입력한 장소와 상대방이 입력한 장소와의 3:3 조합에 의해 최단 거리가 계산됩니다. T.Sharing에서는 재능 키워드가 매칭되는 회원들이 나타나고 최단거리 순으로 정렬됩니다.");
            arrayChild.put(arrayGroup.get(2),arrayList3);

            ArrayList<String> arrayList4 = new ArrayList<String>();
            arrayGroup.add("수준");
            arrayList4.add("관심 재능, 재능 드림 모두 회원님 본인의 수준을 입력 해야 합니다. 총 다섯 개 수준(시작 단계, 초급, 중급, 상급, 전문가)으로 나뉘어집니다. 회원님의 재능 수준은 다른 회원들의 재능 공유를 결정 할 요인이 될 겁니다. 재능 수준을 허위로 입력할 경우 차후 재능 공유 시 신고를 당할 수 있으니 유의 바랍니다.");
            arrayChild.put(arrayGroup.get(3),arrayList4);

            ArrayList<String> arrayList5 = new ArrayList<String>();
            arrayGroup.add("포인트 등록");
            arrayList5.add("회원님의 수준에 따라 포인트를 입력해야 합니다. 재능 드림의 포인트는 재능 공유 후 회원님이 얻는 포인트를 의미하고 관심 재능의 포인트는 재능 공유 후 회원님이 상대방에게 제공하는 포인트를 의미합니다. 재능 드림의 너무 높은 포인트 혹은 관심 재능의 너무 낮은 포인트는 상대방의 재능 공유를 망설이게 할 수 있습니다. 1회 최대 공유포인트는 300포인트로 제한됩니다.");
            arrayChild.put(arrayGroup.get(4),arrayList5);
        }
        else if(value.equals("TSharing"))
        {
            ArrayList<String> arrayList1 = new ArrayList<String>();
            arrayGroup.add("T.Sharing");
            arrayList1.add("회원님과 재능 매칭이 되는 사용자들이 거리 순으로 정렬됩니다. T . Sharing을 통해 매칭 확률이 높은 상대와 재능 공유를 시작하세요.");
            arrayChild.put(arrayGroup.get(0),arrayList1);

            ArrayList<String> arrayList2 = new ArrayList<String>();
            arrayGroup.add("재능 드림 / 관심 재능");
            arrayList2.add("Talented에는 재능 드림과 관심 재능 두 가지 재능이 있습니다. 재능 드림은 회원님의 재능을 가르쳐 줄 수 있는, 공유 할 수 있는 재능을 의미합니다. 반대로 관심 재능은 회원님이 배우고 싶은, 회원님이 관심이 가는 재능을 의미합니다");
            arrayChild.put(arrayGroup.get(1),arrayList2);

            ArrayList<String> arrayList3 = new ArrayList<String>();
            arrayGroup.add("회원 리스트");
        ...
        else if(value.equals("TCondition"))
        {
            ...
            arrayChild.put(arrayGroup.get(1),arrayList1);
        ...
        else if(value.equals("CustomerService"))
        {
            ...
            ArrayList<String> arrayList7 = new ArrayList<String>();
            arrayGroup.add("자주하는 질문");
            arrayList7.add("자주 하는 질문에 대한 문의 내용과 답변 내용이 있습니다. 원하는 내용이 없을 시 1:1 문의하기를 이용하시면 빠른 시일내에 답변을 받을 수 있습니다.");
            arrayChild.put(arrayGroup.get(6),arrayList7);

            ArrayList<String> arrayList8 = new ArrayList<String>();
            arrayGroup.add("1:1 질문하기 / 나의 문의내역");
        ...
        {
            ArrayList<String> arrayList1 = new ArrayList<String>();
            arrayGroup.add("재능공유·관심 내역");
            arrayList1.add("회원님이 공유했던 내역과 주고받은 관심을 확인 할 수 있습니다. 관심, 진행 중인 재능, 진행 취소, 공유 완료된 내역을 확인할 수 있습니다.");
            arrayChild.put(arrayGroup.get(0),arrayList1);

            ArrayList<String> arrayList2 = new ArrayList<String>();
            arrayGroup.add("Profile 보기");
            arrayList2.add("과거에 관심을 주고 받은 상대, 진행했던 상대, 완료했던 상대에게 관심을 보내고 메시지를 보낼 수 있습니다.");
            arrayChild.put(arrayGroup.get(1),arrayList2);

            ArrayList<String> arrayList3 = new ArrayList<String>();
            arrayGroup.add("신고 하기");
            arrayList3.add("공유 내역을 통해 회원님께 비합리적인 행동을 한 상대를 신고할 수 있습니다. 신고하기에 대한 자세한 사항은 \"Talented 사용하기\" → \"고객센터\" → \"신고하기\"에서 확인할 수 있습니다.");
            arrayChild.put(arrayGroup.get(2),arrayList3);

            ArrayList<String> arrayList4 = new ArrayList<String>();
            arrayGroup.add("내역 삭제");
